...
-   **STAGE 3: Implement notification mechanism**
    ...
    -   [:D] Write answers of your learning module's "Reflection Publisher-3" questions in this README.
...
1. Pretty sure this uses the push model. Here, whenever a product is created or deleted, notifications will be generated by it. Full information of the product like the product title and type pushed out from the product to the customers, the push model. 

2. As for the pull model. The notification generated would just be saying something like a new product appeared without saying anything about the product. The observer would then have to ask for the data. This can be good because it only fetches when needed but makes for a tighter coupling.

3. Then it won't run in parallel. Before another notification the previous one must be completed first. It's slow and will get worse as the number of observers increase. Also, it would be stuck doing that task until the end and would be able to handle things that might happen in that timeframe, they'd have to wait until it finishes. 